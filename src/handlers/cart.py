...
async def gen_products(data, user_id):
    ...
    for i in data:
        ...
        keyboard.add(InlineKeyboardButton(text=f'{i[2]}: {i[3]} UAH - {count}шт',
...
async def buy(message: Message):
    ...
    prices = [
        LabeledPrice(
            label=new_data[i][2] + f' x {data[i][3]}',
            amount=new_data[i][3] * 100 * data[i][3]
